...
pub fn read_consensus(project_dir: &Path) -> Result<ConsensusState, String> {
    ...
    for line in content.lines() {
        ...
        if trimmed.starts_with("- **Company**:") {
            company_name = trimmed.trim_start_matches("- **Company**:").trim().to_string();
        } else if trimmed.starts_with("- **Mission**:") {
            mission = trimmed.trim_start_matches("- **Mission**:").trim().to_string();
        } else if trimmed.starts_with("- **Status**:") {
            let s = trimmed.trim_start_matches("- **Status**:").trim().to_uppercase();
            ...
            cycle = trimmed.trim_start_matches("- **Cycle**:").trim().parse().unwrap_or(0);
        } else if trimmed.starts_with("- **Revenue**:") {
            revenue = trimmed.trim_start_matches("- **Revenue**:").trim().to_string();
...
pub fn update_consensus(project_dir: &Path, content: &str) -> Result<(), String> {
    ...
    if path.exists() {
        fs::copy(&path, &backup_path)
            .map_err(|e| format!("Failed to backup consensus: {}", e))?;
    }
